...
getFirestore().doc('_system/features').set({ draftsReady: true, draftsPublic: false, commit: process.env.GITHUB_SHA || null, verifiedAt: new Date() }, { merge: true }).catch(error => { console.error(error); process.exitCode = 1; });
