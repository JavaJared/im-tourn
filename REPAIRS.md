...
4. Run **Deploy Firebase Rules (after frontend)** after confirming the new frontend. This runs the emulator permission tests, applies Firestore and Storage rules, then reads both active Firebase releases and compares their source to the checked-out files. The workflow summary records each active ruleset and source SHA-256. Verification targets the app’s `i-m-tourn.firebasestorage.app` bucket. The service account needs Firebase Rules read access in addition to deployment permissions. The first Storage deployment using Firestore lookups may require enabling cross-service permissions in Firebase. Users with a pre-update tab may need to refresh to use the secured APIs.
