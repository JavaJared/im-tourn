...
## Current username labels

`UserLink` and `UsernameText` resolve the current handle by UID through `getPublicUsernames`. They never fall back to historical display-name snapshots. Lookups deduplicate IDs, use batches of at most 50, cache for one minute, and refresh while mounted. Successful username edits update the shared cache immediately. The public callable returns only usernames for requested IDs; account bios, photo paths, and other private fields are not returned. Missing records or lookup failures use an unavailable label instead of guessing an identity. The friend-list endpoint also hydrates handles from accountProfiles, and submission summaries retain their author IDs for profile links.
